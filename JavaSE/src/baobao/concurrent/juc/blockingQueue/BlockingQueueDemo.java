...
取出(删除)	                     remove()	        poll()	       take()	       poll(timeout, unit)
检查获取当前值(不取出)             element()          peek()


1、阻塞队列的插入方法add、offer和put不能接收null值，否则会抛出NullPointerException
2、阻塞队列的实现类都是线程安全的


 常见的7种阻塞队列：
ArrayBlockingQueue：由数组结构组成的有界阻塞队列，
LinkedBlockingQueue：由链表结构组成的有界（但是默认初始大小为Integer.MAX_VALUE）阻塞队列。
